...
- (void)init:(CDVInvokedUrlCommand* )command {
    ...
    if (command.arguments.count < 2) {
        return [self sendPluginResult:CDVCommandStatus_ERROR command:command message:@"Missing account id"];
    }
    [[InsertManager sharedManager]setVisitorId:command.arguments[0]];
    [[InsertManager sharedManager]setAccountId:command.arguments[1]];
    [self sendPluginResult:CDVCommandStatus_OK command:command];
}
